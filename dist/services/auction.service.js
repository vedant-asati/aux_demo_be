...
class AuctionService {
    static async getAuctionWithBids(auctionId) {
        ...
        if (auction.bidType === client_1.BidType.SEALED && !auction.auctionEnded) {
    ...
    static async determineWinner(auctionId) {
        if (!auctionId || isNaN(auctionId)) {
            throw new Error('Invalid auction ID');
        }
        const auction = await prisma.auction.findUnique({
        ...
        switch (auction.auctionType) {
            ...
            case 'HIGHEST_UNIQUE':
                ...
                const uniqueBids = auction.bids.filter(bid => auction.bids.filter(b => b.amount.equals(bid.amount)).length === 1);
                if (uniqueBids.length > 0) {
                    winningBid = uniqueBids.reduce((highest, current) => {
                        if (!highest)
                            return current;
            ...
            case 'LOWEST_UNIQUE':
                ...
                const uniqueLowBids = auction.bids.filter(bid => auction.bids.filter(b => b.amount.equals(bid.amount)).length === 1);
                if (uniqueLowBids.length > 0) {
                    winningBid = uniqueLowBids.reduce((lowest, current) => {
                        if (!lowest)
                            return current;
    ...
    static async endAuction(auctionId, winnerId) {
        if (!auctionId || isNaN(auctionId)) {
            throw new Error('Invalid auction ID');
        }
    ...
    static async placeBid(auctionId, bidderId, amount) {
        ...
        if (new library_1.Decimal(amount).lt(auction.reservePrice)) {
        ...
        if (auction.bidType === client_1.BidType.OPEN) {
            if (highestBid) {
                ...
                if (auction.auctionType === 'DUTCH' &&
                    new library_1.Decimal(amount).gte(highestBid.amount)) {
        ...
        if (auction.auctionType === 'HIGHEST_UNIQUE' ||
            ...
            const duplicateBid = auction.bids.find(bid => new library_1.Decimal(bid.amount).eq(amount));
            if (duplicateBid) {
